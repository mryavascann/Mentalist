// Portre yer tutucusu: Higgsfield görselleri gelene kadar kişi başına deterministik SVG siluet.
// Renk ve şekil kişi id + ad'dan türer (aynı kişi hep aynı görünür). İfade bilinçli olarak yok:
// yüz ifadesi oyunda ipucu değildir (Barrett 2019). Gerçek görsel geldiğinde bu bileşen base64 WebP döndürür.
import { seedHash } from '@ortak/rastgele';

const TONLAR = ['#8a6d1f', '#3b5b7a', '#7a2a2a', '#2f6b3a', '#5c5247', '#b98b5a', '#6b4e8a', '#8a4a1f'];

export function Portre({ id, ad, boyut = 56, kurban = false }: { id: string; ad: string; boyut?: number; kurban?: boolean }) {
