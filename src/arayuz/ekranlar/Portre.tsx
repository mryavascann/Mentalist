...
export function Portre({ id, ad, boyut = 56, kurban = false, src }: { id: string; ad: string; boyut?: number; kurban?: boolean; src?: string | null }) {
  if (src) {
    return (
      <span style={{ position: 'relative', display: 'inline-block', width: boyut, height: boyut, flex: 'none' }}>
        <img src={src} alt={`${ad} portresi`} width={boyut} height={boyut} style={{ borderRadius: 6, border: '1px solid var(--cizgi)', objectFit: 'cover', display: 'block', opacity: kurban ? 0.55 : 1, filter: kurban ? 'grayscale(.6)' : undefined }} />
        {kurban && <svg width={boyut} height={boyut} viewBox="0 0 64 64" aria-hidden="true" style={{ position: 'absolute', inset: 0 }}><line x1="8" y1="8" x2="56" y2="56" stroke="var(--kirmizi)" strokeWidth="2" /></svg>}
      </span>
    );
  }
  ...
  return (
    <svg width={boyut} height={boyut} viewBox="0 0 64 64" role="img" aria-label={`${ad} portresi (yer tutucu)`} style={{ borderRadius: 6, background: 'var(--kagit-koyu)', border: '1px solid var(--cizgi)', opacity: kurban ? 0.55 : 1 }}>
      ...
      {kurban && <line x1="8" y1="8" x2="56" y2="56" stroke="var(--kirmizi)" strokeWidth="2" />}
    </svg>
  );
}
